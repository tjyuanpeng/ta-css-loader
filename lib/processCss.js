	...
	return function(css) {
		...
		if(options.import) {
			css.walkAtRules("import", function(rule) {
				...
				// ta-css-loader
				// FIX: alienv need module support
...
module.exports = function processCss(inputSource, inputMap, options, callback) {
	...
	// ta-css-loader
	// add new parameter: modulesExclude
	// switch css module mode to global by regular express while mode is 'local'
	// for importing legacy code with global mode
	var _mode = options.mode;
	if (query.modulesExclude && _mode === 'local' && new RegExp(query.modulesExclude).test(options.from)) {
		console.log('@[ta-css-loader][modulesExclude.matched]:', options.from);
		_mode = 'global';
	}
